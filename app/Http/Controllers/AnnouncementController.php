...
class AnnouncementsController extends Controller
{
    ...
    public function index(){
        ...
        return view('dashboard.Announcement.index' ,compact('result'));
    }
    public function search(Request $request)
    {
        ...
        if ($searchTerm) {
            ...
            $all = AnnouncementResource::collection(Announcements::where('status', 1)->orderBy('id', 'DESC')->cursorPaginate(5));
            return $all;
        }
    
        // $results = $query->cursorPaginate(1);
        return AnnouncementResource::collection($query->orderBy('id', 'DESC')->cursorPaginate(5));
    }
...
public function show(Announcements $announcements, $id)
{
    ...
    { 
       $user = Auth::id();
       if( $user && $id == 2){
        $result =  AnnouncementResource::collection(Announcements::where('user_id',$user)->where('status',2)->orderBy('id', 'DESC')->cursorPaginate(5));
        return $result;
        
    }
        elseif( $user && $id == 3){
        $result = AnnouncementResource::collection(Announcements::where('user_id',$user)->where('status',4)->orderBy('id', 'DESC')->cursorPaginate(5));
        return $result;
       }elseif($user && $id == 1){
        $result = AnnouncementResource::collection(Announcements::where('user_id',$user)->orderBy('id', 'DESC')->cursorPaginate(5));
        return $result;
       }
    }
...
{
    // dd($id);
    $announcement = Announcements::where('id', $id)->where(function ($query) {
        $query->orWhere('status', 1)->orWhere('status', 4);
    })->first(); 
    //    dd($announcement);
    if (!$announcement) {
        return response()->json([
            'error' => 'Announcement can`t delete',
        ], 403);
    }
    $announcement->update([
     'status'=>0
    ...
    $announcement->save();
    return response()->json([ 'message'=>'Delete post '],200);
